...
pub const CanonWriter = struct {
    ...
    fn writeStmt(self: *CanonWriter, s: ast.Stmt) !void {
        switch (s) {
            ...
            .insert => |q| {
                ...
                try self.writeInsert(q);
            },
            .update => |q| {
                try self.writeByte(0x03);
                try self.writeUpdate(q);
            },
            .delete => |q| {
                try self.writeByte(0x04);
                try self.writeDelete(q);
            },
            .merge => |q| {
                try self.writeByte(0x05);
                try self.writeMerge(q);
            },
            .create_table => |q| {
                try self.writeByte(0x10);
                try self.writeCreateTable(q);
            },
            .create_index => |q| {
                try self.writeByte(0x11);
                try self.writeCreateIndex(q);
            },
            .alter_table => |q| {
    ...
    fn writeType(self: *CanonWriter, t: ast.SqlType) !void {
        switch (t) {
            ...
            .vector => |dim| {
                try self.writeByte(0x13);
                try self.writeU32(dim);
            },
            .array => |inner| {
                try self.writeByte(0x14);
                try self.writeType(inner.*);
            },
            .struct_type => |fields| {
                try self.writeByte(0x15);
    ...
    fn writeExpr(self: *CanonWriter, e: *ast.Expr) error{OutOfMemory}!void {
        switch (e.*) {
            ...
            .nondet => |k| {
                try self.writeByte(0x08);
                try self.writeByte(@intFromEnum(k));
            },
            .column_ref => |r| {
            ...
            .is_null => |inner| {
                try self.writeByte(0x0D);
                try self.writeExpr(inner);
            },
            .is_not_null => |inner| {
                try self.writeByte(0x0E);
                try self.writeExpr(inner);
            },
            .is_distinct => |pair| {
                try self.writeByte(0x0F);
                try self.writeExpr(pair.left);
                try self.writeExpr(pair.right);
            },
            ...
            .in_subquery => |s| {
                try self.writeByte(0x15);
                try self.writeExpr(s.expr);
                try self.writeSelect(s.query.*);
            },
            .not_in_subquery => |s| {
                try self.writeByte(0x16);
                try self.writeExpr(s.expr);
            ...
            .case_searched => |c| {
                ...
                for (c.whens) |w| {
                    try self.writeExpr(w.cond);
                    try self.writeExpr(w.result);
                }
...
/// SQL in two different databases produces different hashes.
